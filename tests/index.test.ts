...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['WARP_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: WarpAPI) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new WarpAPI({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'WARP_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new WarpAPI({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['WARP_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new WarpAPI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['WARP_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new WarpAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['WARP_API_BASE_URL'] = ''; // empty
      const client = new WarpAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://app.warp.dev/api/v1');
    });

    test('blank env variable', () => {
      process.env['WARP_API_BASE_URL'] = '  '; // blank
      const client = new WarpAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://app.warp.dev/api/v1');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new WarpAPI({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['WARP_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new WarpAPI({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new WarpAPI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new WarpAPI({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new WarpAPI({ apiKey: 'My API Key', fetch: testFetch });
